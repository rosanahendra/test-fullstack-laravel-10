...
<script>
    function sales_website(a) {
        var loading = document.getElementById('sales_website');
        loading.innerHTML = '<div id="sales_website"><div style="margin-top:10px; margin-bottom:20px;"><center style="color:#ff0000;" style="font-size:12px; color:#ff0000;"><i class="fa fa-circle-o-notch fa-pulse"></i></center></div></div>'; 
        $.ajax({
            url: "<?= url('sales_website'); ?>",
            method: "POST",
            data: {
                '_token': '<?= csrf_token() ?>',
                'a': a,
    ...
    //tambahan
    function all_webs() {
        var all_web = document.getElementById("all_web");
        if(all_web.checked == true){
            <?php
            $query = DB::select("select * from account_app_client where acca_status = 'Aktif'");
            foreach($query as $row){ 
            ?>
            document.getElementById("acca_id<?php echo $row->acca_id; ?>").checked = true;
            <?php } ?>
        } else {
            <?php
            $query = DB::select("select * from account_app_client where acca_status = 'Aktif'");
            foreach($query as $row){ 
            ?>
            document.getElementById("acca_id<?php echo $row->acca_id; ?>").checked = false;
            <?php } ?>
        }
    }
    
    function non_all_website() {
        document.getElementById("all_web").checked = false;        
    }
    
    function akses_menu_member(a) {
        var loading = document.getElementById('akses_menu_member');
        loading.innerHTML = '<div id="akses_menu_member"><div style="margin-top:10px; margin-bottom:20px;"><center style="color:#ff0000;" style="font-size:12px; color:#ff0000;"><i class="fa fa-circle-o-notch fa-pulse"></i></center></div></div>'; 
        $.ajax({
            url: "<?= url('akses_menu_member'); ?>",
            method: "POST",
            data: {
                '_token': '<?= csrf_token() ?>',
                'a': a,
    ...
    function validation_password(a, b) {
        if(a != ''){
            $.ajax({
                url: "<?= url('validation_password'); ?>",
                method: "POST",
                data: {
                    '_token': '<?= csrf_token() ?>',
                    'a': a,
                ...
                success: function(response) {
                    ...
                    if(message == true){
                        document.getElementById('message_'+b).innerHTML = '<div id="message_'+b+'"></div>';
                        document.getElementById('button_save').innerHTML = '<div id="button_save"><input type="submit" value="Simpan" name="save" class="btn btn-primary" /></div>';
                    } else {
                        document.getElementById('message_'+b).innerHTML = '<div id="message_'+b+'"><div class="info_alert">'+message+'</div></div>';
                        document.getElementById('button_save').innerHTML = '<div id="button_save"><input type="submit" value="Simpan" name="save" class="btn btn-primary" disabled /><div class="info_alert">'+message+'</div></div>';
                    }
                }
            });
        } else {
    ...
    function validation_email_edit(a, b, c) {
        $.ajax({
            url: "<?= url('validation_email_edit'); ?>",
            method: "POST",
            data: {
                '_token': '<?= csrf_token() ?>',
                'a': a,
    ...
    function validation_whatsapp(b) {
        var a = document.getElementById('memb_phone_code').value;
        var c = document.getElementById('memb_phone').value;
        $.ajax({
            url: "<?= url('validation_whatsapp'); ?>",
            method: "POST",
            data: {
                '_token': '<?= csrf_token() ?>',
                'a': a,
    ...
    function validation_whatsapp_edit(b, d) {
        var a = document.getElementById('memb_phone_code').value;
        var c = document.getElementById('memb_phone').value;
        $.ajax({
            url: "<?= url('validation_whatsapp_edit'); ?>",
            method: "POST",
            data: {
                '_token': '<?= csrf_token() ?>',
                'a': a,
