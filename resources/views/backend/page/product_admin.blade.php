...
<script>
 ...
 function member_jenis(a, b) {
    var loading = document.getElementById('memb_code'+b);
    loading.innerHTML = '<div id="memb_code'+b+'"><div style="margin-top:10px; margin-bottom:20px;"><center style="color:#ff0000;" style="font-size:12px; color:#ff0000;"><i class="fa fa-circle-o-notch fa-pulse"></i></center></div></div>'; 
    $.ajax({
        url: "<?= url('member_jenis'); ?>",
        method: "POST",
        data: {
            '_token': '<?= csrf_token() ?>',
            'a': a,
        ...
        success: function(data) {
            $("#memb_code"+b).html(data);
        }
    });
 }

 function member_jenise(a, b) {
    var loading = document.getElementById('memb_codee'+b);
    loading.innerHTML = '<div id="memb_codee'+b+'"><div style="margin-top:10px; margin-bottom:20px;"><center style="color:#ff0000;" style="font-size:12px; color:#ff0000;"><i class="fa fa-circle-o-notch fa-pulse"></i></center></div></div>'; 
    $.ajax({
        url: "<?= url('member_jenise'); ?>",
        method: "POST",
        data: {
            '_token': '<?= csrf_token() ?>',
            'a': a,
 ...
 function add_patient() {
   ...
   add.innerHTML += '<div id="removepatient'+no+'"><div class="row"> <div class="col-lg-4 col-md-4 col-sm-6 col-12"> <div class="form-group"> <?php $var_name = 'copr_type'; $label = 'Jenis Komisi'; ?> <label for="basicInput"><?php echo $label; ?> <font class="font-required">Wajib</font></label> <select name="<?php echo $var_name; ?>[]" class="form-control" onchange="member_jenis(this.value, '+no+')" required> <option value="">Pilih <?php echo $label; ?></option> <?php $var = ''; ?> <option value="Komisi Affiliate">Komisi Affiliate</option> <option value="Komisi Vendor">Komisi Vendor</option> <option value="Komisi Partner">Komisi Partner</option> <option value="Komisi Instalasi">Komisi Instalasi</option> </select><div id="message_<?php echo $var_name; ?>"></div> </div> </div> <div class="col-lg-4 col-md-4 col-sm-6 col-12"> <div class="form-group"> <div id="memb_code'+no+'"> <?php $var_name = 'memb_code'; $label = 'Pilih Nama Pengguna'; ?> <label for="basicInput"><?php echo $label; ?> <font class="font-required">Wajib</font></label> <select name="<?php echo $var_name; ?>[]" class="form-control autocom" required> <option value="">Pilih <?php echo $label; ?></option> <?php foreach($content1 as $row){ $var = ''; ?> <option value="<?php echo $row->memb_code; ?>"> <?php echo $row->memb_name.' (ID Pelanggan : '.$row->memb_code.' - No WA : '.$row->memb_phone_code.$row->memb_phone; ?> </option> <?php } ?> </select> </div> </div> </div> <div class="col-lg-3 col-md-3 col-sm-5 col-10"> <div class="form-group"> <?php $var_name = 'copr_value'; $label = 'Nilai Komisi'; $var = ''; ?> <label for="basicInput"><?php echo $label; ?> <font class="font-required">Wajib</font></label> <input class="form-control" required name="<?php echo $var_name; ?>[]" placeholder="<?php echo $label; ?>" type="number"><div id="message_<?php echo $var_name; ?>"></div> </div> </div> <div class="col-lg-1 col-md-1 col-sm-1 col-2"><div class="form-group"><label for="basicInput">Aksi</label><div><a style="cursor:pointer;font-family: INHERIT;font-weight: bold; color:#fff;font-size: 12px;" id="more_fields" onclick="removepatient('+no+');" class="btn btn-danger"><i class="fa-solid fa-xmark"></i></a></div></div></div> <div class="row"> <div class="col-lg-12"> <div style="border:1px solid #cccccc; width:100%; margin-top:10px; margin-bottom:15px;"></div> </div> </div></div>';
